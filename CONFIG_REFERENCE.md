...
| Ключ | Default | Описание |
|---|---|---|
| `BACKUP_TARGET` | `bot` (fallback) | `panel` (Remnawave, рекомендуется) или `bot` (rwp_shop). Отсутствие ключа = `bot` (обратная совместимость со старыми установками); новым установкам цель предлагает мастер первого запуска — по умолчанию **панель** |
...
| `PANEL_BILLING_BACKUP` | `auto` | `auto` = дампить если запущен; установлен, но не запущен — бэкап без биллинга с предупреждением и пометкой в Telegram · `true` = обязателен (иначе провал бэкапа) · `false` = выкл |
...
| `BOT_KB_BACKUP` | `auto` | `auto` = дампить если запущен; установлен, но не запущен — пропуск с предупреждением и пометкой в Telegram · `true` = обязателен (для удалённого по SSH бота не поддерживается — бэкап откажет) · `false` = выкл |
...
| `MAX_BACKUP_SIZE_MB` | `0` | лимит суммарного объёма архивов `lazarus_*` (0 = без лимита); снапшоты отката и прочие файлы каталога не считаются и не удаляются |
