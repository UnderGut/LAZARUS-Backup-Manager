...
```bash
# Установка одной командой (DEV-версия с миграцией)
curl -sSL https://raw.githubusercontent.com/UnderGut/LAZARUS-Backup-Manager/dev/install.sh | bash
...
```bash
# Обновите до dev-версии с миграцией
cd /opt/lazarus-backup
git fetch origin
git checkout dev
git pull origin dev
