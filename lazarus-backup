...
# --- НАСТРОЙКИ ---
VERSION="4.6.2-pipe-install-fix"
SCRIPT_NAME="lazarus"
...
install_script() {
    if [[ "$EUID" -ne 0 ]]; then
        echo -e "${RED}Ошибка: Запустите скрипт с правами root (sudo)!${RESET}"
        exit 1
    fi
    ...
    # Проверка: если скрипт уже запущен из папки установки - ничего не делаем
    if [[ "$CURRENT_SCRIPT_PATH" == "$INSTALLED_SCRIPT_PATH" ]]; then
        return
    fi

    echo -e "${CYAN}--- УСТАНОВКА LAZARUS BACKUP ---${RESET}"
    if [[ ! -d "$INSTALL_DIR" ]]; then
        mkdir -p "$INSTALL_DIR"
        print_message "SUCCESS" "Создана папка: $INSTALL_DIR"
    fi

    # Логика копирования или скачивания
    if [[ -f "$0" && "$0" != "bash" ]]; then
        # Если скрипт запущен как файл (./lazarus)
        cp "$(realpath "$0")" "$INSTALLED_SCRIPT_PATH"
    else
        # Если скрипт запущен через pipe (curl | bash)
        print_message "INFO" "Загрузка скрипта с GitHub..."
        if command -v curl >/dev/null 2>&1; then
            curl -sL "$REMOTE_URL" -o "$INSTALLED_SCRIPT_PATH"
        elif command -v wget >/dev/null 2>&1; then
            wget -qO "$INSTALLED_SCRIPT_PATH" "$REMOTE_URL"
        else
            print_message "ERROR" "Для установки нужен curl или wget!"
            exit 1
        fi
    fi
    ...
    print_message "SUCCESS" "Установка завершена. Запуск..."
    echo ""
...
uninstall_script() {
    clear
    echo -e "${RED}${BOLD}УДАЛЕНИЕ СКРИПТА${RESET}"
    echo "Вы собираетесь удалить LAZARUS Backup Manager."
    echo ""
    
    read -erp "Напишите 'yes' для подтверждения (Enter - Отмена): " confirm
    ...
    echo ""
    read -erp "Удалить все сохраненные бэкапы с диска? (y/N): " del_backups

    (crontab -l 2>/dev/null | grep -v "$SCRIPT_NAME") | crontab -
    print_message "SUCCESS" "Задачи Cron удалены."

    rm -f "$SYMLINK_PATH"
    print_message "SUCCESS" "Симлинк удален."

    if [[ "$del_backups" =~ ^[Yy]$ ]]; then
        rm -rf "$INSTALL_DIR"
        print_message "SUCCESS" "Скрипт, конфиг и БЭКАПЫ полностью удалены."
    else
        rm -f "$INSTALL_DIR/$SCRIPT_NAME" "$CONFIG_FILE"
        rmdir "$BACKUP_DIR" 2>/dev/null
        rmdir "$INSTALL_DIR" 2>/dev/null
        print_message "SUCCESS" "Скрипт удален."
        if [[ -d "$BACKUP_DIR" ]]; then
            print_message "WARN" "Ваши бэкапы остались в папке: $BACKUP_DIR"
        fi
    fi
    echo ""
    exit 0
...
get_bot_version_display() {
    ...
    if [[ -n "$ver" ]]; then echo "$ver"; else echo "Не определена / Остановлен"; fi
}

detect_bot_installation() {
    print_message "INFO" "Запуск интеллектуального поиска..."
    scan_system_for_bot 
    if [[ -n "$FOUND_PATH" ]]; then
        echo ""; print_message "SUCCESS" "Обнаружена установка!"
        echo -e "   Путь:      ${BOLD}$FOUND_PATH${RESET}"
        [[ -n "$FOUND_BOT" ]] && echo -e "   Бот:       ${BOLD}$FOUND_BOT${RESET}"
        [[ -n "$FOUND_DB" ]]  && echo -e "   БД:        ${BOLD}$FOUND_DB${RESET}"
        echo ""
        read -erp "Использовать эти настройки? (Y/n): " confirm
        if [[ -z "$confirm" || "$confirm" =~ ^[Yy]$ ]]; then
            BOT_PATH="$FOUND_PATH"
            if [[ -n "$FOUND_BOT" ]]; then BOT_CONTAINER_NAME="$FOUND_BOT"; fi
            if [[ -n "$FOUND_DB" ]]; then DB_CONTAINER_NAME="$FOUND_DB"; fi
            save_config; print_message "SUCCESS" "Настройки сохранены."; return 0
        fi
    fi
    print_message "WARN" "Авто-поиск не дал результата. Введите данные вручную в Настройках."
    return 1
}
    ...
    if [[ -n "$BOT_CONTAINER_NAME" && "$IGNORE_MISMATCH" != "true" ]]; then
        if ! docker inspect "$BOT_CONTAINER_NAME" &>/dev/null; then
            ...
            if [[ -n "$FOUND_BOT" && "$FOUND_BOT" != "$BOT_CONTAINER_NAME" ]]; then
                echo ""; print_message "WARN" "Контейнер '$BOT_CONTAINER_NAME' не найден, но найден '$FOUND_BOT'."
                echo -e " ${GREEN}Предлагаю обновить настройки.${RESET}"
                read -erp "Обновить? (Y/n): " upd
                if [[ -z "$upd" || "$upd" =~ ^[Yy]$ ]]; then
                    BOT_CONTAINER_NAME="$FOUND_BOT"
                    [[ -n "$FOUND_DB" ]] && DB_CONTAINER_NAME="$FOUND_DB"
                    [[ -n "$FOUND_PATH" ]] && BOT_PATH="$FOUND_PATH"
                    save_config; print_message "SUCCESS" "Обновлено."; sleep 1
                else
...
ensure_bot_path() {
    if [[ -z "$BOT_CONTAINER_NAME" || -z "$DB_CONTAINER_NAME" ]]; then
        echo ""; print_message "WARN" "Настройки контейнеров пусты."
    ...
    if [[ "$path_valid" == "false" || "$container_valid" == "false" || "$db_valid" == "false" ]]; then
        ...
        if [[ -z "$BOT_PATH" || ! -d "$BOT_PATH" ]]; then
            echo ""; print_message "ACTION" "Укажите путь к папке бота вручную."
            read -erp "Путь (Enter для '$DEFAULT_BOT_PATH'): " input_path
            BOT_PATH="${input_path:-$DEFAULT_BOT_PATH}"
            if [[ ! -d "$BOT_PATH" ]]; then print_message "ERROR" "Папка не найдена!"; return 1; fi
            save_config
        fi

        if [[ -z "$BOT_CONTAINER_NAME" ]] || ! docker inspect "$BOT_CONTAINER_NAME" &>/dev/null; then
            echo ""; print_message "WARN" "Контейнер бота не определен."
            print_message "ACTION" "Введите имя контейнера бота (как в 'docker ps')."
            read -erp "Имя: " input_name
        ...
        if [[ -z "$DB_CONTAINER_NAME" ]] || ! docker inspect "$DB_CONTAINER_NAME" &>/dev/null; then
            echo ""; print_message "WARN" "Контейнер Базы Данных не найден."
            print_message "ACTION" "Введите имя контейнера БД (или Enter для '$DEFAULT_DB_NAME')."
            read -erp "Имя БД: " input_db
...
load_or_create_config() {
    ...
    if [[ -z "$BOT_TOKEN" || -z "$CHAT_ID" ]]; then
        echo ""; print_message "WARN" "Настройки Telegram не найдены."
        [[ -z "$BOT_TOKEN" ]] && read -erp " Введите Bot Token: " BOT_TOKEN
        [[ -z "$CHAT_ID" ]] && read -erp " Введите Chat ID: " CHAT_ID
        [[ -z "$TG_MESSAGE_THREAD_ID" ]] && read -erp " Введите ID топика (или Enter): " TG_MESSAGE_THREAD_ID
        updated=true
    fi
...
send_telegram_document() {
    ...
    print_message "INFO" "Отправка в Telegram..."
    ...
    if [[ "$http_code" == "200" ]]; then
        print_message "SUCCESS" "Успешно отправлено в Telegram."
    else
        local file_size=$(stat -c%s "$file_path" 2>/dev/null || echo 0)
        
        if [[ $file_size -ge 52428800 ]]; then
            print_message "ERROR" "Файл >50MB. Telegram API отклонил отправку."
            local error_msg="⚠️ *Ошибка отправки бэкапа*

Файл занимает больше 50 МБ\. Telegram Bot API не позволяет загружать такие файлы напрямую\.

📂 *Бэкап сохранен локально:*
\`$(basename "$file_path")\`"
            ...
            print_message "ERROR" "Не удалось отправить в Telegram (HTTP код: $http_code)."
        fi
    fi
}
...
create_backup() {
    ...
    if [[ "$TYPE" != "db_only" ]]; then
        ...
        if [[ "$SKIP_COUNT" -gt 0 ]]; then
            ...
            print_message "WARN" "Пропущено больших файлов (>${MAX_FILE_SIZE_MB}MB): $SKIP_COUNT (общий вес: $SKIP_SIZE)"
            EXCLUDE_FLAGS+=(--exclude-from="$SKIP_FILE")
            SKIP_INFO=$'\n⚠️ Skip: '"$SKIP_COUNT"$' (>'"$MAX_FILE_SIZE_MB"$'MB)'
        fi
    fi
    ...
    if [[ "$TYPE" == "db_only" ]]; then
        ...
        print_message "INFO" "Архивация файлов..."
    ...
    # --- НОВЫЙ ФОРМАТ СООБЩЕНИЯ В TELEGRAM ---
    # Используем стандартное серверное время, как просил пользователь ("вернем назад")
    ...
    case "$TYPE" in
        "full")
            HASHTAG="💾 #full_backup"
            TYPE_NAME="📦 Полный бэкап"
            ;;
        "db_only")
            HASHTAG="💾 #db_only"
            TYPE_NAME="🗄 Дамп базы данных"
            ;;
        "files_only")
            HASHTAG="💾 #files_only"
            TYPE_NAME="📂 Файлы бота"
            ;;
    esac

    local VER_MSG=""; [[ -n "$CURRENT_VER" ]] && VER_MSG=" | 🏷 v${CURRENT_VER}"
    
    local CAPTION="${HASHTAG}
📅 ${DISPLAY_DATE}
${TYPE_NAME}
📏 ${SIZE}${VER_MSG}${SKIP_INFO}"
...
select_backup_file() {
    ...
    while true; do
        if ! compgen -G "$BACKUP_DIR/${FILTER_PREFIX}_*.tar.gz" > /dev/null; then
            print_message "WARN" "Файлов типа '${TITLE}' не найдено."; read -erp "Enter..." dummy; return 1
        fi
        ...
        clear; echo -e "${GREEN}Восстановление: ${BOLD}${TITLE}${RESET}"
        echo -e "Всего файлов: ${CYAN}${total_count}${RESET}"; echo ""
        ...
        for (( i=0; i<loop_limit; i++ )); do
            ...
            local VER_STR=""
            if [[ "$B_VER" == "Unknown" || "$B_VER" == "" ]]; then VER_STR="${GRAY}[Н/Д]${RESET}"
        ...
        if [[ $total_count -gt 5 ]]; then
            if [[ $loop_limit -lt $total_count ]]; then
                 echo -e " ${MAGENTA}6. Показать все остальные ($((total_count - 5)) шт)...${RESET}"
            else
                 echo -e " ${MAGENTA}6. Свернуть список (показать 5)${RESET}"
            fi
        fi

        if [[ "$show_filenames" == "false" ]]; then
            echo -e " 7. Показать имена файлов"; 
        else
            echo -e " 7. Показать дату и размер"; 
        fi
        echo " 0. Назад"; echo ""
        read -erp "Номер (Enter - Назад): " choice
        ...
        else print_message "ERROR" "Неверный выбор."; sleep 1; fi
    done
}

execute_restore() {
    local MODE="$1"; local FILE="$2"
    echo ""; print_message "WARN" "ВНИМАНИЕ: Текущие данные будут перезаписаны!"
    read -erp "Продолжить? (y/n): " confirm; [[ "$confirm" != "y" ]] && return

    local TMP_DIR="$BACKUP_DIR/restore_tmp_$$"
    mkdir -p "$TMP_DIR"
    print_message "INFO" "Распаковка..."
    ...
    if [[ "$MODE" == "full" && -n "$DIR_ARC" ]]; then
        ...
        print_message "INFO" "Очистка папки..."
        rm -rf "$BOT_PATH"/*
        print_message "INFO" "Извлечение файлов..."
        tar -xzf "$DIR_ARC" -C "$(dirname "$BOT_PATH")"
        docker volume rm "$DB_VOLUME_NAME" 2>/dev/null || true
        print_message "INFO" "Запуск БД..."
        docker compose up -d "$DB_SERVICE_NAME"
        sleep 5
    fi

    if [[ -n "$DB_DUMP" ]]; then
        print_message "INFO" "Импорт базы данных..."
        if ! ensure_bot_path; then rm -rf "$TMP_DIR"; return; fi 
    ...
    rm -rf "$TMP_DIR"
    print_message "SUCCESS" "Восстановление успешно завершено."
...
menu_manual_backup() {
    while true; do
        clear; echo -e "${GREEN}Создание ручного бэкапа${RESET}"
        echo " 1. Полный бэкап (БД + Файлы)"
        echo " 2. Только База Данных"
        echo " 3. Только Файлы"
        echo " 0. Назад"
        echo ""
        read -erp "Ваш выбор (Enter - Назад): " m_choice
        [[ -z "$m_choice" ]] && return
...
menu_restore() {
    while true; do
        ...
        clear; echo -e "${GREEN}Меню восстановления${RESET}"
        echo -e " 1. Полный бэкап      ${c_f}[${n_full} шт]${RESET}"
        echo -e " 2. Только База       ${c_d}[${n_db} шт]${RESET}"
        echo -e " 3. Только Файлы      ${c_fl}[${n_files} шт]${RESET}"
        echo " 0. Назад"
        echo ""
        read -erp "Выбор (Enter - Назад): " r_choice
        [[ -z "$r_choice" ]] && return

        local SELECTED_FILE=""
        case $r_choice in
            1) if select_backup_file "lazarus_full" "Полные бэкапы"; then execute_restore "full" "$SELECTED_FILE"; fi ;;
            2) if select_backup_file "lazarus_db" "Бэкапы БД"; then execute_restore "db_only" "$SELECTED_FILE"; fi ;;
            3) if select_backup_file "lazarus_files" "Бэкапы Файлов"; then 
                   echo ""; print_message "WARN" "Будут перезаписаны файлы в папке бота!"
                   read -erp "Продолжить? (y/n): " cf
                   if [[ "$cf" == "y" ]]; then 
                       if ensure_bot_path; then tar -xzf "$SELECTED_FILE" -C "$(dirname "$BOT_PATH")"; print_message "SUCCESS" "Файлы распакованы."; read -erp "Enter..." dummy; fi
                   fi
               fi ;;
            0) return ;;
        esac
...
setup_cron_task() {
    ...
    clear; echo -e "${GREEN}Настройка периода: ${BOLD}${TASK_TYPE^^}${RESET}"
    echo " 1. Ежечасно"
    echo " 2. Ежедневно (04:00)"
    echo " 3. Еженедельно"
    echo " 4. Свое время"
    echo " 5. Каждые N минут"
    echo " 6. Отключить"
    echo ""
    read -erp "Выбор (Enter - Назад): " c
    [[ -z "$c" ]] && return
    
    local NEW_CRON_LINES=""; local DISPLAY_TIME=""

    case $c in
        1) NEW_CRON_LINES="0 * * * * $JOB_CMD >> /var/log/lazarus_backup.log 2>&1 $JOB_ID"; DISPLAY_TIME="Ежечасно" ;;
        2) NEW_CRON_LINES="0 4 * * * $JOB_CMD >> /var/log/lazarus_backup.log 2>&1 $JOB_ID"; DISPLAY_TIME="Ежедневно 04:00" ;;
        3) NEW_CRON_LINES="0 4 * * 1 $JOB_CMD >> /var/log/lazarus_backup.log 2>&1 $JOB_ID"; DISPLAY_TIME="Еженедельно" ;;
        4) echo "Время (ЧЧ:ММ) через пробел:"; read -erp "> " times_input
           if [[ -z "$times_input" ]]; then print_message "ERROR" "Время не введено!"; sleep 1; return; fi
           for t in $times_input; do if [[ "$t" =~ ^([0-9]{1,2}):([0-9]{1,2})$ ]]; then h=$((10#${BASH_REMATCH[1]})); m=$((10#${BASH_REMATCH[2]})); NEW_CRON_LINES+="$m $h * * * $JOB_CMD >> /var/log/lazarus_backup.log 2>&1 $JOB_ID"$'\n'; fi; done; DISPLAY_TIME="Свое: $times_input" ;;
        5) read -erp "Введите интервал (минут): " interval
           if [[ "$interval" =~ ^[0-9]+$ ]] && [ "$interval" -gt 0 ] && [ "$interval" -lt 60 ]; then
               NEW_CRON_LINES="*/$interval * * * * $JOB_CMD >> /var/log/lazarus_backup.log 2>&1 $JOB_ID"
               DISPLAY_TIME="Каждые $interval мин"
           else print_message "ERROR" "Введите число от 1 до 59."; sleep 1; return; fi ;;
        6) DISPLAY_TIME="Выкл" ;;
        *) return ;;
    esac

    local CURRENT_CRON=$(crontab -l 2>/dev/null | grep -v "$JOB_ID")
    if [[ "$DISPLAY_TIME" != "Выкл" && -n "$NEW_CRON_LINES" ]]; then echo -e "$CURRENT_CRON\n$NEW_CRON_LINES" | crontab -
    ...
    save_config; print_message "SUCCESS" "Расписание обновлено!"; sleep 1
}

menu_automation() {
    while true; do
        clear; echo -e "${GREEN}Настройка Автоматического Бекапа${RESET}"
        echo -e " 1. Полный бекап [${YELLOW}${SCHEDULE_FULL}${RESET}]"
        echo -e " 2. Только База  [${YELLOW}${SCHEDULE_DB}${RESET}]"
        echo -e " 3. Только Файлы [${YELLOW}${SCHEDULE_FILES}${RESET}]"
        echo " 0. Назад"
        echo ""
        read -erp "Выбор (Enter - Назад): " ac
        [[ -z "$ac" ]] && return
...
menu_settings() {
    while true; do
        clear; echo -e "${GREEN}Настройки скрипта${RESET}"
        local MASKED_TOKEN="${BOT_TOKEN:0:5}.......${BOT_TOKEN: -5}"; [[ ${#BOT_TOKEN} -lt 10 ]] && MASKED_TOKEN="*******"
        echo -e " 1. Путь к боту:      ${CYAN}${BOT_PATH}${RESET}"
        echo -e " 2. Telegram Token:   ${CYAN}${MASKED_TOKEN}${RESET}"
        echo -e " 3. Chat ID:          ${CYAN}${CHAT_ID}${RESET}"
        echo -e " 4. Thread ID:        ${CYAN}${TG_MESSAGE_THREAD_ID:-Нет}${RESET}"
        echo -e " 5. Пользователь БД:  ${CYAN}${DB_USER}${RESET}"
        echo -e " 6. Хранить бэкапы:   ${CYAN}${RETAIN_BACKUPS_DAYS} дней${RESET}"
        echo -e " 7. Имя контейнера:   ${CYAN}${BOT_CONTAINER_NAME}${RESET}"
        echo -e " 8. Контейнер БД:     ${CYAN}${DB_CONTAINER_NAME}${RESET}"
        echo -e " 9. Исключить папки:  ${CYAN}${EXCLUDE_DIRS:-Нет}${RESET}"
        echo -e " 10. Макс. размер файла: ${CYAN}${MAX_FILE_SIZE_MB} MB${RESET}"
        echo " 0. Назад"
        echo ""
        read -erp "Что изменить (Enter - Назад): " s
        [[ -z "$s" ]] && return
        
        case $s in
            1) read -erp "Новый путь: " np; if [[ -n "$np" ]]; then if [[ -d "$np" ]]; then BOT_PATH="$np"; save_config; print_message "SUCCESS" "Сохранено"; else print_message "ERROR" "Папка не найдена!"; fi; fi ;;
            2) read -erp "Новый Token: " nt; if [[ -n "$nt" ]]; then BOT_TOKEN="$nt"; save_config; print_message "SUCCESS" "Сохранено"; fi ;;
            3) read -erp "Новый Chat ID: " nid; if [[ -n "$nid" ]]; then CHAT_ID="$nid"; save_config; print_message "SUCCESS" "Сохранено"; fi ;;
            4) read -erp "Новый Thread ID: " ntid; TG_MESSAGE_THREAD_ID="$ntid"; save_config; print_message "SUCCESS" "Сохранено" ;;
            5) read -erp "Новый DB User: " ndb; if [[ -n "$ndb" ]]; then DB_USER="$ndb"; save_config; print_message "SUCCESS" "Сохранено"; fi ;;
            6) read -erp "Дней хранения: " nd; if [[ "$nd" =~ ^[0-9]+$ ]]; then RETAIN_BACKUPS_DAYS="$nd"; save_config; print_message "SUCCESS" "Сохранено"; fi ;;
            7) read -erp "Имя контейнера бота: " nc; if [[ -n "$nc" ]]; then BOT_CONTAINER_NAME="$nc"; save_config; print_message "SUCCESS" "Сохранено"; fi ;;
            8) read -erp "Имя контейнера БД: " nc; if [[ -n "$nc" ]]; then DB_CONTAINER_NAME="$nc"; save_config; print_message "SUCCESS" "Сохранено"; fi ;;
            9) read -erp "Исключить (например: uploads/cache node_modules): " ed; 
               if [[ -n "$ed" ]]; then 
                   EXCLUDE_DIRS="$ed"
                   save_config
                   print_message "SUCCESS" "Сохранено"
               else
                   if [[ -n "$EXCLUDE_DIRS" ]]; then
                        read -erp "Очистить список исключений? (y/N): " clr
                        if [[ "$clr" =~ ^[Yy]$ ]]; then EXCLUDE_DIRS=""; save_config; print_message "SUCCESS" "Очищено"; fi
                   fi
               fi ;;
            10) read -erp "Макс. размер файла в MB (целое число): " msize
                if [[ "$msize" =~ ^[0-9]+$ ]] && [ "$msize" -gt 0 ]; then
                    MAX_FILE_SIZE_MB="$msize"; save_config; print_message "SUCCESS" "Сохранено"
                else print_message "ERROR" "Введите число больше 0"; fi ;;
            0) return ;;
...
cleanup_old_backups() {
    clear; echo -e "${RED}Очистка старых бэкапов${RESET}"; echo "Поиск файлов старше $RETAIN_BACKUPS_DAYS дней..."
    local COUNT=$(find "$BACKUP_DIR" -name "lazarus_*.tar.gz" -mtime +$RETAIN_BACKUPS_DAYS | wc -l)
    if [[ "$COUNT" -eq 0 ]]; then print_message "INFO" "Файлов не найдено."; read -erp "Enter..." dummy; return; fi
    find "$BACKUP_DIR" -name "lazarus_*.tar.gz" -mtime +$RETAIN_BACKUPS_DAYS -print
    echo ""; print_message "WARN" "Найдено файлов: $COUNT"; read -erp "Удалить? (y/n): " c
    if [[ "$c" == "y" ]]; then
        find "$BACKUP_DIR" -name "lazarus_*.tar.gz" -mtime +$RETAIN_BACKUPS_DAYS -delete
        find "$BACKUP_DIR" -name "lazarus_*.tar.gz.version" -mtime +$RETAIN_BACKUPS_DAYS -delete
        print_message "SUCCESS" "Очищено."; else print_message "INFO" "Отменено."; fi
    read -erp "Enter..." dummy
}
...
install_script
load_or_create_config
check_config_mismatch # Проверка при запуске
...
case "$1" in
    ...
    *)
        while true; do
            ...
            echo -e "Версия бота: ${CYAN}${BOT_VERSION}${RESET}"
            echo ""
            echo -e "Статусы авто-бекапа:"
            echo -e " • Full:  ${YELLOW}${SCHEDULE_FULL}${RESET}"
            echo -e " • DB:    ${YELLOW}${SCHEDULE_DB}${RESET}"
            echo -e " • Files: ${YELLOW}${SCHEDULE_FILES}${RESET}"
            echo ""
            echo " --- ДЕЙСТВИЯ ---"
            echo " 1. Ручной бекап (Full / DB / Files)"
            echo " 2. Настроить авто-бекап"
            echo " 3. Восстановить из бекапа"
            echo " 4. Настройки (Пути / Токены / Ротация)"
            echo " 5. Удалить старые бэкапы (>${RETAIN_BACKUPS_DAYS} дней)"
            echo ""
            echo -e "${RED} 666. Удалить скрипт (Uninstall)${RESET}"
            echo " 0. Выход"
            echo ""
            read -erp "Выбор (Enter - выход): " opt; [[ -z "$opt" ]] && exit 0
            case $opt in
